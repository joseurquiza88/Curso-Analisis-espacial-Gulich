...
class(base)
head(base)
#tranformar de un dataframe a un sf
base_t <- st_as_sf(base, coords = c("x", "y"), crs = 4326)
class(base_t)# ahora es  "sf" "data.frame"
base_t

#### Conversión de coordenadas geográficas a planas (UTM) ####
#codigos utm de Argentina ==> 32720 faja 20 32721 faja 21, 32719 faja 19
# UTM vs Posgar no hay motivo para usar una que otra
# Desde el punto estadistico no hay diferencia
base_t <- st_transform(base_t, crs = 32720) #faja 20
base_t

#### Exportar base de datos ####
#lo guardamos en csv, le agregamos una columna con la goemetria
st_write(base_t,
         "soja_t.csv",
         layer_options = "GEOMETRY=AS_XY",#agrega columna de goemetria
         delete_layer = T) # .csv
st_write(base_t, "soja_t.shp", delete_layer = T) # .shp
st_write(base_t, "soja_t.gpkg", delete_layer = T) # .gpkg
# delete_layer, esto hace que si ya esta guardad/exportdao
# el archivo lo pisa
